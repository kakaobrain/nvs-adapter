...
@threestudio.register("nvsadapter-system")
class NvsAdapter(BaseLift3DSystem):
    @dataclass
    ...
    def training_step(self, batch, batch_idx):
        if self.cfg.freq.get("ref_or_nvsadapter", "accumulate") == "accumulate":
            do_ref = True
            do_nvsadapter = True
        elif self.cfg.freq.get("ref_or_nvsadapter", "accumulate") == "alternate":
            do_ref = (
                self.true_global_step < self.cfg.freq.ref_only_steps
                or self.true_global_step % self.cfg.freq.n_ref == 0
            )
            do_nvsadapter = not do_ref

        total_loss = 0.0
        if do_nvsadapter:
            out = self.training_substep(batch, batch_idx, guidance="nvsadapter")
            total_loss += out["loss"]
